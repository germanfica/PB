class Cajero {
  ...
  retirar(monto) {
    ...
    while (i<=this.billetes.length-1 && !todoOK) {
      ...
      if(cantBilletes>0) {
        arregloCantBilletesUsad.push(cantBilletesReq); // Cantidad de billetes usados para concretar la operación
        ...
        if (dineroRestante == 0) {
          todoOK = true;
          this.darBilletes(arregloCantBilletesUsad);
        }
      }
      i++;
    }
